<script lang="ts">
  ...
  /**
   * Shared strategy-population table, reused by both the ETF scanner
   * (`/population`) and the equity scanner (`/equity/population`). The route
   * supplies the windowed `population` and scanner-specific heading copy.
   */
  interface Props {
    population: Strategy[];
    title: string;
  ...
  let { population, title, subtitle }: Props = $props();

  const promotedCount = $derived(population.filter((p) => p.status === 'promoted').length);
  ...
  const columns: Column<Strategy>[] = [
    ...
    },
    {
      ...
      header: 'Precision',
      numeric: true,
      sortValue: (r) => r.latestScore?.precisionOos ?? -Infinity,
      cell: precision
    },
    {
      id: 'pf',
...
{#snippet precision(r: Strategy)}
  {#if r.latestScore?.precisionOos != null}
    {@const v = r.latestScore.precisionOos}
    <span
      class="rounded px-1 font-semibold {v >= 0.5 ? 'bg-up/15 text-up' : 'text-base-200'}"
      title="OOS precision P(profit | acted) at τ*={fmtUnit(
        r.latestScore.actThreshold ?? 0.5
      )} over n={r.latestScore.nActed ?? 0} acted trades"
    >
      {(v * 100).toFixed(0)}%
    </span>
  {:else}
...
<PageHeader {title} {subtitle}>
  {#snippet actions()}
    ...
    <span class="num rounded border border-base-800 bg-base-900/60 px-2 py-1 text-xs text-base-300">
      {promotedCount} promoted / {population.length} total
    </span>
  {/snippet}
</PageHeader>
